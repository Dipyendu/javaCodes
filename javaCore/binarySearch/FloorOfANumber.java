...
public class FloorOfANumber{
    public static void main(String[] args){
        ...
        System.out.println("\nFloor -> " + floor(arr1, target));
        System.out.println("\nFloor -> " + floor(arr2, target));
    }
    ...
    public static int floor(int[] arr, int target) {
        ...
        if(isAscending){
            ...
            // -> when start > end i.e start = end + 1
            if(start < arr.length){
                // -> start < arr.length is to check for when target element is smaller than all elements present in the array and array is not seaching for an start index which is out of bound
                return arr[start];
            }
            System.out.println("\nTarget element is smaller than all elements present in the array so just printing the smallest element present in the array");
