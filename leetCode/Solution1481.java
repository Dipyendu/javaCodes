...
public class Solution1481 {
    public static void main(String[] args) {
        ...
        System.out.println(findLeastNumOfUniqueIntsOptimized(arr, k));
    }
...
//        Count the frequency of each element
        ...
        for (int num : arr) {
            numberTable.put(num, numberTable.getOrDefault(num, 0) + 1);
        }
        // Store the values in an array
        List<Integer> values = new ArrayList<>(numberTable.values());

        // Sort the values in increasing order
        Collections.sort(values);

        int i = 0;
        while (k >= 0 && i < values.size()) {
            k = k - values.get(i);
            if (k >= 0) {
                values.remove(i);
            }
    ...
    public static int findLeastNumOfUniqueIntsOptimized(int[] arr, int k) {
