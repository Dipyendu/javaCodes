...
public class Solution0042 {
    public static void main(String[] args) {
        ...
        System.out.println(trapWithoutComments(height));
    }
        ...
        while (left < right) {
            ...
            if (height[left] < height[right]) {
                ...
                if (height[left] >= maxLeft) {
                    ...
                    // maximum, as anymore will overflow it. We also subtract the current height, as that is the elevation the
                    // ground will be at.
                } else {
    ...
    public static int trapWithoutComments(int[] height) {
        if (height == null || height.length == 0) {
            return 0;
        }
        int left = 0;
        ...
        while (left < right) {
            if (height[left] < height[right]) { // -> If left height < right height -> left++
                if (height[left] >= maxLeft) { // if current left height >= previous max left -> update maxLeft
                    maxLeft = height[left];
                } else { // else add it to sum
                    totalWater += maxLeft - height[left];
                }
                left++;
            } else { // -> If right height <= left height -> right--
                if (height[right] >= maxRight) { // if current right height >= previous max right -> update maxRight
                    maxRight = height[right];
                } else { // else add it to sum
                    totalWater += maxRight - height[right];
                }
